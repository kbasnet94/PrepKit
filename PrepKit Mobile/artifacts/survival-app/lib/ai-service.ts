...
function buildOnDeviceSystemPrompt(context: KnowledgeContext, mode: QueryMode): string {
  ...
  let prompt = `You are PrepKit, an offline survival and emergency preparedness assistant. Your job is to give practical, actionable guidance.\n\n`;
...
export async function generateAIResponse(
  ...
  try {
    ...
    const sources: SourceRef[] = [
      ...context.actionCards.map((c) => ({ title: "PrepKit Quick Guide", section: c.title })),
      ...context.chunks
