---
name: prepkit-validation
description: >
  Use this skill to validate a PrepKit guide draft for schema correctness, safety issues, duplicate risk, unsupported claims, missing warnings/red flags, taxonomy fit, and constraint metadata correctness. Trigger whenever the user asks to validate a guide, check a guide for errors, review a draft before import, or run quality checks on guide content. Also trigger when a guide-annotated.json needs validation before staging. This is step 5 of the pipeline: Planning → Research → Writing → Constraint Annotation → **Validation** → Import/Staging → Human Review → Release.
---

# PrepKit Validation / Safety Skill
